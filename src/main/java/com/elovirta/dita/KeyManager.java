...
import static com.elovirta.dita.Utils.isLocalDita;
...
public class KeyManager {
  ...
  public void read(URI uri, XdmNode map) {
    ...
    var refs =
        map.select(descendant(isElement().and(isLocalDita())).then(attribute(HREF_ATTR)))
        ...
        ref -> {
          ...
          try (var in = Utils.readFile(ref); ) {
            for (var line = in.readLine(); line != null; line = in.readLine()) {
              ...
              while (matcher.find()) {
                logger.info("keyref value={}, start={} end={}",
                        matcher.group(2),
